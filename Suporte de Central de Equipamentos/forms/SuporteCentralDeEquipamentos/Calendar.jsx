...
class Calendario extends React.Component {
    ...
    getData() {
        ...
        DatasetFactory.getDataset(
            "AgendamentoCursosCentral",
            null,
            [
                DatasetFactory.createConstraint("operacao", "Select", "Select", ConstraintType.MUST),
                DatasetFactory.createConstraint("dia", dia, dia, ConstraintType.MUST),
                DatasetFactory.createConstraint("WKNumProces", $("#numProces").val() != "" ? $("#numProces").val() : 0, $("#numProces").val() != "" ? $("#numProces").val() : 0, ConstraintType.MUST)
            ],
            null,
            {
    ...
    atualizaCalendario(agendamentos) {
        var horarios = ["09:00", "10:00", "11:00", "14:00", "15:00", "16:00"];
        var calendario = this.state.calendario.slice();
    ...
    render() {
        ...
        if (
            this.props.curso != "" &&
            this.props.obra != "" &&
            (this.state.agendado.horario != "" || parseInt(this.state.agendado.horario) == 0) &&
            (this.state.agendado.dia != "" || parseInt(this.state.agendado.dia) == 0)
        ) {
            if (
                this.state.calendario[this.state.agendado.horario][this.state.agendado.dia].curso != this.props.curso ||
                this.state.calendario[this.state.agendado.horario][this.state.agendado.dia].obra != this.props.obra
            ) {
                var calendario = this.state.calendario.slice();
            ...
            row.forEach((column, i) => {
                ...
                tr.push(
                    <td
                        ...
                        className={column.status == 2 ? "success" : (column.status == 1 ? "info" : column.status == 3 ? "warning" : "") + " cell"}
                        style={{ color: "white", maxWidth: "18vw", overflow: "hidden" }}
                    >
...
function RelatorioDeSolicitacoes({ }) {
    ...
    async function BuscaListagemDeAgendamentos() {
        var ds = await BuscaDados();
                ...
                <tbody>
                    {ds.map((item, index) => {
                        return (<tr key={index}>
                            ...
                            <td>{item.CURSO}</td>
                            <td style={{ whiteSpace: "nowrap" }}>{item.DIA.split("-").reverse().join("/") + " - " + item.HORARIO}</td>
                            <td>{item.SOLICITACAO}</td>
                            <td>{ExtraiListaDeUsuarios(item.USUARIOS)}</td>
                        </tr>);
                    })}
                </tbody>
            </table>
        ...
        function ExtraiListaDeUsuarios(usuarios) {
            usuarios = JSON.parse(usuarios);

            var list = [];

            usuarios.map((usuario, index) => {
                list.push(index + "º:" + usuario.nome);
            });
        ...
        function BuscaDados() {
            return new Promise((resolve, reject) => {
                DatasetFactory.getDataset(
                    "AgendamentoCursosCentral",
                    null,
                    [DatasetFactory.createConstraint("operacao", "ListagemDeAgendamentos", "ListagemDeAgendamentos", ConstraintType.MUST)],
                    null,
                    {
                        success: (dataset) => {
    ...
    async function BuscaQuantidadeDeAgendamentoPorObra() {
        var ds = await BuscaDados();
        ...
        setHtmlRelatorio(
            <table className="table table-bordered">
                ...
                <tbody>
                    {ds.map((item, index) => {
                        var popoverContent = "<h4>" + item.OBRA + "</h4>";
                        for (const Agendamento of item.AGENDAMENTOS) {
                            popoverContent += "<div style='white-space:nowrap'><b>" + Agendamento.CURSO + ":</b> " + Agendamento.QNTD + "</div>";
                        }


                        return (<tr key={index} className="bs-docs-popover-hover" data-content={popoverContent}>
                            <td>{item.OBRA}</td>
                            <td>{item.COUNT}</td>
                        </tr>);
                    })}
        ...
        function BuscaDados() {
            return new Promise((resolve, reject) => {
                DatasetFactory.getDataset(
                    "AgendamentoCursosCentral",
                    null,
                    [DatasetFactory.createConstraint("operacao", "ContagemDeAgendamentosPorObra", "ContagemDeAgendamentosPorObra", ConstraintType.MUST)],
                    null,
                    {
                        success: (dataset) => {
                            var list = [];
                            ...
                            for (const Item of dataset.values) {
                                var found = list.find((element) => element.OBRA == Item.OBRA);

                                if (found) {
                                    found.COUNT += parseInt(Item.QNTD);
                                    found.AGENDAMENTOS.push(Item);
                                } else {
                                    list.push({
                                        OBRA: Item.OBRA,
                                        COUNT: parseInt(Item.QNTD),
                                        AGENDAMENTOS: [Item]
                                    })
                                }
                            }
    ...
    function RenderRelatorio() {
        if (relatorio == "Listagem de Agendamentos Realizados") {
            BuscaListagemDeAgendamentos();
        }
        else if (relatorio == "Quantidade de Agendamentos Por Obra") {
            BuscaQuantidadeDeAgendamentoPorObra();
        }
    }
    ...
    return (
        <div>
            <Panel Title="Filtro">
                <label htmlFor="SelectRelatorio">Relatório:</label>
                <select name="SelectRelatorio" id="SelectRelatorio" className="form-control" onChange={(e) => setRelatorio(e.target.value)} value={relatorio}>
                    <option value=""></option>
                    <option value="Listagem de Agendamentos Realizados">Listagem de Agendamentos Realizados</option>
                    <option value="Quantidade de Agendamentos Por Obra">Quantidade de Agendamentos Por Obra</option>
                </select>
            </Panel>
            <br />
            <Panel Title="Relatório">{HtmlRelatorio}</Panel>
        </div>
    );
}
