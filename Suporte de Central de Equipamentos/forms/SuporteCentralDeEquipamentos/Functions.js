...
function ValidaCampos() {
    ...
    if (atividade == 0 || atividade == 4) {
        ...
        if ($("#categoria").val() == "Alteração Motivo da Falha") {
            var justificativa = $("#justificativaAlteracao")
            if (justificativa.val().length < 15) {
                justificativa.addClass("has-error");
                valida = false;
                FLUIGC.toast({
                ...
                $([document.documentElement, document.body]).animate({
                    scrollTop: justificativa.offset().top - (screen.height * 0.15)
                }, 700);
            } else {
                justificativa.removeClass("has-error");
            }
        }
...
function InsereLinhaAlteracaoMotivo() {
    var html =
        "<tr>\
            <td>\
                <textarea class='form-control descricaoMateriais inputInfoChamado'></textarea>\
            </td>\
            <td>\
                <textarea class='form-control motivoFalha inputInfoChamado'></textarea>\
            </td>\
            <td style='text-align: center;'>\
                <button class='btn btn-danger btnRemoveLinha'>\
                    <i class='flaticon flaticon-trash icon-sm' aria-hidden='true'></i>\
    ...
    $("#tableAlteracao>tbody").append(html);
    $("#tableAlteracao>tbody").find("tr:last").find(".btnRemoveLinha").on("click", function () {
        $(this).closest("tr").remove();
    });
}

function SalvaDadosAlteracao() {
    const alteracoes = [];
    $("#tableAlteracao>tbody>tr").each(function () {
        const descricao = $(this).find(".descricaoMateriais").val();
        const motivo = $(this).find(".motivoFalha").val();
        alteracoes.push({
            descricao: descricao,
            motivo: motivo,
        });
    });
    $("#inputAlteracoesJSON").val(JSON.stringify(alteracoes));
}

function CriaListaAlteracoes() {
    const json = JSON.parse($("#inputAlteracoesJSON").val());
    $("#tableAlteracao thead th:last-child").hide();
    for (const alteracao of json) {
        const html =
            "<tr>\
                    <td>\
                        <span>" + (alteracao.descricao ? alteracao.descricao : " - ") + "</span>\
                    </td>\
                    <td>\
                        <span>" + (alteracao.motivo ? alteracao.motivo : " - ") + "</span>\
                    </td>\
                </tr>";
        $("#tableAlteracao>tbody").append(html);
    }
    ...
    DatasetFactory.getDataset("FCFO", ["CGCCFO, NOMEFANTASIA"], [
        ...
        success: (fornecedores) => {
            if (fornecedores.columns[0] == "error") {
                ...
                var optSelected = $("#selectFornecedor").val();
                $("#selectFornecedor").html("<option></option>");
                fornecedores.values.forEach(fornecedor => {
                    $("#selectFornecedor").append("<option value='" + fornecedor.CGCCFO + " - " + fornecedor.NOMEFANTASIA + "'>" + fornecedor.CGCCFO + " - " + fornecedor.NOMEFANTASIA + "</option>")
                });
                $("#selectFornecedor").val(optSelected);
                $('#selectFornecedor').select2({
                    heigth: "34px",
                    width: "100%",
                    language: "select2/i18n/pt-BR",
